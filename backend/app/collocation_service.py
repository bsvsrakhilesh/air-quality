...
class CollocationService:
    ...
    def _best_lag(series: pd.Series, reference: pd.Series, max_lag: int) -> tuple[int, float | None]:
        ...
        for lag in range(-max_lag, max_lag + 1):
            ...
            paired = pd.concat([shifted.diff(), reference.diff()], axis=1).dropna()
    ...
    def analyze(self, request: CollocationAnalysisRequest) -> dict[str, Any]:
        ...
        # A preliminary fleet median makes lag suggestions independent of filenames or upload order.
        preliminary = combined.median(axis=1, skipna=True)
        ...
        for sensor in sensors:
            name = sensor.name
            peers = analysis.drop(columns=[name]).median(axis=1, skipna=True)
            pair = pd.concat([analysis[name].rename("sensor"), peers.rename("consensus")], axis=1).dropna()
            x = pair["sensor"].to_numpy(dtype=float)
            y = pair["consensus"].to_numpy(dtype=float)
            residual = x - y
            mean_reference = float(np.mean(np.abs(y))) if len(y) else 0.0
            relative_bias = float(np.mean(residual) / mean_reference * 100) if mean_reference > 1e-12 else None
            rmse = float(np.sqrt(np.mean(residual**2))) if len(residual) else math.nan
            nrmse = float(rmse / mean_reference * 100) if mean_reference > 1e-12 else None
            ...
            completeness = float(pair.shape[0] / len(analysis) * 100)
            ...
            checks = {
                ...
                "nrmse": nrmse is not None and nrmse <= gates.nrmse,
                "relative_bias": relative_bias is not None and abs(relative_bias) <= gates.relative_bias,
                "slope": gates.slope_min <= slope <= gates.slope_max,
            }
            status = "Ready" if all(checks.values()) else "Review"
            ...
            series.append(
                {
                    ...
                    "consensus": _number(float(np.median(valid_values)) if valid_values else None),
                }
            )
        ...
        icc_single, icc_average = _icc_absolute(analysis)
        row_means = analysis.mean(axis=1).abs()
        fleet_cv = analysis.std(axis=1, ddof=1).div(row_means.where(row_means > 1e-12)).mul(100)
        ...
        if duration_hours < 24 * 30:
            warnings.append("This session is shorter than 30 days; treat drift findings as provisional.")

        return {
            "study": {"name": request.study_name, "parameter": request.parameter_name, "unit": request.unit},
            "summary": {
                ...
                "ready_sensors": ready,
                "status": "Ready" if ready == len(sensors) else "Review needed",
